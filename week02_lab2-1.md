...
void main() {
  ...
  // ==============================
  // ข้อ 2 หาวิชาที่ได้คะแนนสูงสุด
  // ==============================
  var highest =
      courseScores.entries.reduce((a, b) => a.value > b.value ? a : b);
  ...
  // ==============================
  // ข้อ 4 สร้าง Set เก็บวิชาที่ผ่าน (>=80)
  // ==============================
...
void main() {
  ...
  // === ขั้นตอนที่ 3 : กรองเฉพาะคณะวิศวกรรม ===
  print("\n=== นักศึกษาคณะวิศวกรรม ===");
  var engineeringStudents =
      students.where((s) => s["faculty"] == "วิศวกรรม").toList();
  ...
  // ข้อ 1 : หานักศึกษาที่ GPA สูงสุดในแต่ละคณะ
...
// ==================================================
// Function : หานักศึกษาที่ GPA สูงสุดในคณะที่ระบุ
// ==================================================
...
// บันทึกโค้ดในส่วนนี้
// =========================================
// ข้อ 1 : BankAccount และ CheckingAccount
// =========================================
...
class BankAccount {
  ...
  void deposit(double amount) {
    balance += amount;
    print("$owner ฝากเงิน $amount บาท");
  }

  void withdraw(double amount) {
    if (amount <= balance) {
      balance -= amount;
      print("$owner ถอนเงิน $amount บาท");
    } else {
...
class CheckingAccount extends BankAccount {
  CheckingAccount(String owner, double balance) : super(owner, balance);

  @override
  void withdraw(double amount) {
    // อนุญาตให้ติดลบได้ไม่เกิน 500 บาท
    if (balance - amount >= -500) {
      balance -= amount;

      // ถ้ามีการ Overdraft คิดค่าธรรมเนียม 50 บาท
      if (balance < 0) {
        balance -= 50;
        print("ถอนเงินแบบ Overdraft ค่าธรรมเนียม 50 บาท");
      }
...
abstract class Vehicle {
  ...
  void drive(double km) {
    ...
    if (used <= fuel) {
      fuel -= used;
      print("ขับรถ $km กม. ใช้น้ำมัน ${used.toStringAsFixed(2)} ลิตร");
    } else {
      print("น้ำมันไม่พอ");
    }
  }
...
class Product with Discountable {
  ...
  void discount(double percent) {
    ...
    print("ลดราคา $percent%");
    print("ราคาใหม่ $price บาท");
  }
}
...
void main() {
  ...
  var account = CheckingAccount("สมชาย", 1000);
  account.showBalance();

  account.withdraw(1300); // Overdraft
  account.showBalance();

  account.withdraw(300); // เกินวงเงิน
  account.showBalance();
  ...
  Product phone = Product("iPhone", 35000);
  phone.showPrice();
  phone.discount(10);
}
...
```
บันทึกผลการทดลอง:
Sequential ใช้เวลา: 1909 ms
Parallel ใช้เวลา:   802 ms
ประหยัดเวลาได้:     1107 ms ( 58 %)
```
...
void main() async {
  ...
  for (int i = 0; i < incomes.length; i++) {
    print(
        "ผู้ใช้ ${i + 1} รายได้ ${incomes[i]} บาท → ภาษี ${taxes[i]} บาท");
    totalTax += taxes[i];
  }

  print("ภาษีรวมทั้งหมด: $totalTax บาท");
...
```text
ใช้ final เมื่อค่าจะถูกกำหนดเพียงครั้งเดียว แต่ทราบค่าตอนโปรแกรมกำลังทำงาน เช่น วันที่ปัจจุบัน หรือข้อมูลจาก API
ใช้ const เมื่อค่าคงที่และทราบตั้งแต่ก่อนรันโปรแกรม เช่น ค่า π, จำนวนวันในสัปดาห์ หรือข้อความคงที่ต่าง ๆ

```
**ข้อ 2** Named Parameters และ Positional Parameters ต่างกันอย่างไร? ควรเลือกใช้แบบไหนเมื่อไหร่?
```text
ใช้ Positional Parameters เมื่อมีพารามิเตอร์น้อยและลำดับชัดเจน ส่วน Named Parameters เมื่อมีหลายพารามิเตอร์ ต้องการให้โค้ดอ่านง่ายและป้องกันการส่งค่าผิดลำดับ

```
**ข้อ 3** Abstract Class และ Mixin มีจุดประสงค์ต่างกันอย่างไร? ยกตัวอย่างสถานการณ์ที่เหมาะกับแต่ละแบบ
```text
ใช้ Abstract Class เมื่อต้องการกำหนดมาตรฐานของคลาสลูก และใช้ Mixin เมื่อต้องการแชร์ความสามารถให้หลายคลาสโดยไม่ต้องใช้การสืบทอดหลายชั้น
...
```text
Sequential เหมาะกับงานที่มีลำดับและข้อมูลของขั้นตอนก่อนจำเป็นต่อขั้นตอนถัดไป
Parallel เหมาะกับงานที่เป็นอิสระต่อกัน สามารถทำพร้อมกันได้ จึงช่วยลดเวลาการทำงานโดยรวมได้อย่างมาก

```
**ข้อ 5** Future และ Stream ต่างกันอย่างไร? ยกตัวอย่างสถานการณ์ที่เหมาะกับแต่ละแบบจากการพัฒนา Mobile App จริงๆ
```text
Future ใช้สำหรับงานที่ประมวลผลแล้วส่งผลลัพธ์กลับมา เพียงครั้งเดียว ในอนาคต เช่น การเข้าสู่ระบบ (Login), การดึงข้อมูลผู้ใช้จาก API หรือการบันทึกข้อมูลลงฐานข้อมูล เมื่อทำงานเสร็จแล้วก็จะจบการทำงาน
Stream ใช้สำหรับงานที่มีการส่งข้อมูลกลับมา หลายครั้งอย่างต่อเนื่อง เหมาะกับข้อมูลที่เปลี่ยนแปลงตลอดเวลา เช่น ข้อความแชทแบบเรียลไทม์, การติดตามตำแหน่ง GPS, การแจ้งเตือน หรือข้อมูลจากเซนเซอร์
